...
from novelspider.items import ResearchspiderItem
import re
...
class newsSpider(RedisSpider):
    ...
    name = "researchspider"
    redis_key = 'researchspider:start_urls'

    start_urls = []
    for stockCode in STOCKCODES:
        url =  'http://guba.eastmoney.com/list,' + stockCode + ',2,f_' + '1.html'
    ...
    def parse(self,response):
        ...
        for stockCode in STOCKCODES:
            if int(List[2])*int(List[3])<int(List[1]):
                # 生成页面链接
                nextLink = 'http://guba.eastmoney.com/list,' + stockCode + ',2,f_' + str(int(List[3])+1) + '.html'
                # 抓取页面并且处理
